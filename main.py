...
def start_by_category(category_name: str, db: Session = Depends(get_db)):
    category = db.query(Category_Complete_Sentence).filter(Category_Complete_Sentence.name == category_name).first()
    if not category:
        return {"error": "Kategori tidak ditemukan"}

    first_question = db.query(Complete_Sentence)\
                       .filter(Complete_Sentence.Id_ccs == category.Id_ccs)\
...
async def answer(request: Request, db: Session = Depends(get_db)):
    ...
    # ambil Id_clw dari soal sekarang
    current_category_id = item.Id_ccs

    # ambil soal selanjutnya hanya dalam kategori yang sama
    next_item = db.query(Complete_Sentence)\
                  .filter(Complete_Sentence.Id_ccs == current_category_id, Complete_Sentence.Id_cs > id_cs)\
                  .order_by(Complete_Sentence.Id_cs).first()
